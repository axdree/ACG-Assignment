...
#USERNAME: acgadmin
#PASSWORD: ftpP@$$w0rd
import base64, time, datetime, ftplib, io, random, getpass, requests, socket, os, pickle, configparser, hashlib
...
scriptfile = open(scriptpath, "rb")
filehash = hashlib.sha1()
scriptdata = scriptfile.read()
filehash.update(scriptdata)
scriptDigest = filehash.hexdigest()
scriptfile.close()
...
config.read(os.path.dirname(os.path.abspath(__file__)) + "/CAMERA_ID.ini")
CAMERA_ID = config.get("ID", "CAMERAID")
...
def initialConn():
    ...
    hashrecvd = conn.recv(4096).decode("utf-8")
    if "Listening for hash" in hashrecvd:
        conn.send(scriptDigest.encode())
    ...
    if "Listening for status" in received:
        ...
        print("Connection Failed. Please try restarting the app.")
        exit()
